package org.ali.tutorial.controller;

import java.util.ArrayList;
import java.util.List;

import org.ali.tutorial.domain.Person;
import org.ali.tutorial.domain.PersonList;
import org.ali.tutorial.util.Writer;
...
public class RestClientController {
	...
	public void getPhoto(@RequestParam("id") Long id, HttpServletResponse response) {
		...
		ResponseEntity<byte[]> result = restTemplate.exchange("http://localhost:8080/SpringRest/ali/person/{id}", HttpMethod.GET, entity, byte[].class, id);
	...
	public String getAll(Model model) {
		...
		// Send the request as GET
		try {
			ResponseEntity<PersonList> result = restTemplate.exchange("http://localhost:8080/SpringRest/ali/persons", HttpMethod.GET, entity, PersonList.class);
			// Add to model
	...
	public String getPerson(@RequestParam("id") Long id, Model model) {
		...
		// Send the request as GET
		try {
			ResponseEntity<Person> result = restTemplate.exchange("http://localhost:8080/SpringRest/ali/person/{id}", HttpMethod.GET, entity, Person.class, id);
			// Add to model
	...
	public String addPerson(@ModelAttribute("personAttribute") Person person,
		...
		try {
			ResponseEntity<Person> result = restTemplate.exchange("http://localhost:8080/SpringRest/ali/person", HttpMethod.POST, entity, Person.class);
		} catch (Exception e) {
			logger.error(e);
		}
	...
	public String updatePerson(@ModelAttribute("personAttribute") Person person,
		...
		ResponseEntity<String> result = restTemplate.exchange("http://localhost:8080/SpringRest/ali/person/{id}", HttpMethod.PUT, entity, String.class, id);
	...
	public String deletePerson(@RequestParam("id") Long id,
		...
		ResponseEntity<String> result = restTemplate.exchange("http://localhost:8080/SpringRest/ali/person/{id}", HttpMethod.DELETE, entity, String.class, id);
